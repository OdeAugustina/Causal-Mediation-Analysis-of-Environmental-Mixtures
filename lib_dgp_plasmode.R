#!/usr/bin/env Rscript
###########################################################
## lib_dgp_plasmode.R (v3) -- NHANES-anchored plasmode cells
##
## CELLS
...
## The contrast sets every mixture component to its marginal Q25 or Q75
## simultaneously.  That vector is a defined intervention, not an observed
## row, so report how far it sits from the observed joint support.  This is
## the same contrast the synthetic cells use, so the numbers are directly
## comparable across the grid.
pm_intervention_support <- function(Z, a, astar) {
  d_hi <- sqrt(rowSums(sweep(Z, 2, a,     "-")^2))
  d_lo <- sqrt(rowSums(sweep(Z, 2, astar, "-")^2))
  c(nearest_hi = min(d_hi), nearest_lo = min(d_lo),
    pct_within1_hi = 100*mean(d_hi <= 1), pct_within1_lo = 100*mean(d_lo <= 1),
    pct_within1.5_hi = 100*mean(d_hi <= 1.5),
    pct_within1.5_lo = 100*mean(d_lo <= 1.5),
    mean_offdiag_corr = mean(stats::cor(Z)[upper.tri(diag(ncol(Z)))]))
}
...
pm_build_spec <- function(sid, cfg = PM_CFG, verbose = TRUE) {
  ...
  ## rank deficiency can NA any coefficient, not just the confounder block
  alpha_C[is.na(alpha_C)] <- 0; beta_C[is.na(beta_C)] <- 0
  alpha_Z[is.na(alpha_Z)] <- 0; beta_Z[is.na(beta_Z)]  <- 0
  if (is.na(beta_Mf)) stop("fitted beta_M is NA -- mediator dropped from the outcome fit")
  if (max(abs(beta_Z)) == 0)
    stop("all beta_Z are zero; the interaction direction v is undefined")
  ...
  cA <- cell$amp_alpha
  alpha_Z2 <- alpha_Z * cA
  ## Var(Z a_Z + C a_C) INCLUDING the cross-covariance.  Computing it as
  ## var(Za) + var(Ca) drops 2*Cov(Za, Ca), which is nonzero whenever the
  ## exposures and confounders are correlated (they are, in NHANES).
  ## With the n-1 divisor the OLS identity Var(M) = Var(fitted) + Var(resid)
  ## holds exactly, so at cA = 1 this must return the fitted residual SD --
  ## asserted below.
  mu_M_lin <- as.numeric(Z %*% alpha_Z2 + C %*% alpha_C)
  var_mu_M <- stats::var(mu_M_lin)
  varZ     <- as.numeric(t(alpha_Z2) %*% stats::cov(Z) %*% alpha_Z2)  # reporting only
  s2       <- varM_obs - var_mu_M
  if (s2 <= 0)
    stop(sprintf("amp_alpha = %.2f exhausts the mediator variance budget ", cA),
         sprintf("(explained share %.1f%%). Lower it.", 100*var_mu_M/varM_obs))
  sigma_M2 <- sqrt(s2)
  if (isTRUE(all.equal(cA, 1.0)) &&
      !isTRUE(all.equal(sigma_M2, sigma_M, tolerance = 1e-8)))
    stop(sprintf("variance-budget check failed at cA=1: %.10f vs fitted %.10f",
                 sigma_M2, sigma_M))
  ...
  sp <- list(alpha_0 = alpha_0b, alpha_Z = alpha_Z2, alpha_C = alpha_C,
             ...
             fitted = list(alpha_Z = alpha_Z, beta_M = beta_Mf,
                           sigma_M = sigma_M, varZ_share = varZ/varM_obs),
             provenance = list(
               built_at = Sys.time(), R_version = R.version.string,
               source_file = "lib_dgp_plasmode.R v3",
               sheet = cfg$sheet, data_path = cfg$data_path,
               n_base_complete = base$n_base, L = base$L, P = base$P,
               exposures = cfg$exposures, confounders = cfg$confounders,
               mediator = cfg$mediator, outcome = cfg$outcome,
               Z_center = attr(scale(as.matrix(base$Z)), "scaled:center"),
               seed_formula = "SIM_CONFIG$seed + scenario_id*1000 + rep_id",
               n_plasmode = cfg$n_plasmode, lambda_int = cfg$lambda_int))

  full <- pm_truth_on_support(Z, C, sp, n_int = 200L, seed = 20260807L)
  mix  <- rowSums(Z)
  isup <- pm_intervention_support(Z, a, astar)
  sp$diagnostics <- list(corr_mix_M_obs = stats::cor(mix, base$M_obs),
                         truth_full = full, varZ_share = varZ/varM_obs,
                         explained_share = var_mu_M/varM_obs,
                         intervention_support = isup)
  ...
  if (verbose) {
    ...
    cat(sprintf("sigma_M %.5f (fitted %.5f) | explained share of Var(M) %.1f%%\n",
                sigma_M2, sigma_M, 100*var_mu_M/varM_obs))
    cat(sprintf("intervention support: nearest row to Q75 %.3f SD (%.1f%% within 1SD); Q25 %.3f (%.1f%%)\n",
                isup["nearest_hi"], isup["pct_within1_hi"],
                isup["nearest_lo"], isup["pct_within1_lo"]))
...
pm_support_diagnostic <- function(rep_id = 1L, sid, sp = NULL, cfg = PM_CFG) {
  ...
  ## Z is already standardised on the FULL base; re-scaling within the
  ## replicate would silently redefine the mixture index.
  mix <- rowSums(d$Z)
...
cat("[plasmode] lib_dgp_plasmode.R v3 loaded (cells 13 null / 14 detectable)\n")
